...
def _evaluate_one_selection(features: np.array, labels: np.array, model_function, feature_selection_to_test: list,
                            metric=mean_absolute_error, modeltype: str = 'RF', shuffle: bool = False) -> float:
    ...
    result = model_function(train_features=train_feat, train_labels=train_labels, test_features=test_feat, modeltype=modeltype)
    error = metric(result, test_labels)
    return error


def greedy_feature_selection(features: np.array, labels: np.array, model_function, modeltype: str = 'RF', metric=mean_absolute_error,
    ...
    for i in range(max_solution_length):
        ...
        for next_feature_index in range(remaining_features.shape[1]):
            if next_feature_index in current_selection:
                print("Feature "+str(next_feature_index)+ " is already used in ", current_selection)
            else:
                print("Testing with next feature", next_feature_index)

                feature_selection_to_test = current_selection + [next_feature_index]
                error = _evaluate_one_selection(features=features, labels=labels, model_function=model_function, modeltype=modeltype, metric=metric,
